...
func cmdLibrary(args []string) int {
	...
	default:
		fmt.Fprintf(errOut, "Unknown library subcommand: %s\n", subcmd)
		printLibraryUsage()
		return ExitError
	}
}
...
func cmdLibraryIndex(args []string) int {
	...
	default:
		fmt.Fprintf(errOut, "Unknown index subcommand: %s\n", subcmd)
		printLibraryIndexUsage()
		return ExitError
	}
}
...
`)
	...
	if err != nil {
		fmt.Fprintf(errOut, "Error: invalid path: %v\n", err)
		return ExitError
	}
	...
	if err != nil {
		fmt.Fprintf(errOut, "Error: cannot access path: %v\n", err)
		return ExitError
	}
	if !info.IsDir() {
		fmt.Fprintf(errOut, "Error: path is not a directory: %s\n", absPath)
		return ExitError
	}
	...
	if err != nil {
		fmt.Fprintf(errOut, "Error: failed to generate index: %v\n", err)
		return ExitError
	}
	...
	if err != nil {
		fmt.Fprintf(errOut, "Error: failed to create output file: %v\n", err)
		return ExitError
	}
	defer outFile.Close()
	...
	if err != nil {
		fmt.Fprintf(errOut, "Error: failed to write index: %v\n", err)
		return ExitError
	}
...
`)
	...
	if *keyFlag == "" {
		fmt.Fprintf(errOut, "Error: -key flag is required\n")
		fs.Usage()
		return ExitError
	}
	...
	if err != nil {
		fmt.Fprintf(errOut, "Error: failed to load index: %v\n", err)
		return ExitError
	}
	...
	if err != nil {
		fmt.Fprintf(errOut, "Error: failed to load private key: %v\n", err)
		return ExitError
	}

	// Sign
	if err := index.Sign(privateKey, *keyIDFlag); err != nil {
		fmt.Fprintf(errOut, "Error: failed to sign index: %v\n", err)
		return ExitError
	}
...
`)
	...
	if *pubkeyFlag == "" {
		fmt.Fprintf(errOut, "Error: -pubkey flag is required\n")
		fs.Usage()
		return ExitError
	}
	...
	if err != nil {
		fmt.Fprintf(errOut, "Error: failed to load public key: %v\n", err)
		return ExitError
	}

	// Verify
	if err := index.Verify(publicKey); err != nil {
		fmt.Fprintf(errOut, "❌ Signature verification FAILED: %v\n", err)
		return ExitError
	}
...
`)
	...
	if err != nil {
		fmt.Fprintf(errOut, "Error: failed to generate key pair: %v\n", err)
		return ExitError
	}
	...
	if err := library.SavePrivateKey(privateKeyPath, privateKey); err != nil {
		fmt.Fprintf(errOut, "Error: failed to save private key: %v\n", err)
		return ExitError
	}
	...
	if err := library.SavePublicKey(publicKeyPath, publicKey); err != nil {
		fmt.Fprintf(errOut, "Error: failed to save public key: %v\n", err)
		return ExitError
	}
