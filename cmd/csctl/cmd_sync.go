...
func cmdSync(url, token string, args []string, cfg *config.Config, clientOpts []api.ClientOption, rulePath string) int {
	...
	if len(loadErrors) > 0 {
		...
		fmt.Fprintln(errOut, "Warning: skipping files with errors (-force-sync):")
	...
	if len(pushRules) > 0 {
		...
		if !vResult.Valid() {
			...
			fmt.Fprintln(errOut, "Warning: continuing despite validation errors (-force-sync)")
		}
	}
	...
	if len(toPush) > 0 {
		...
		if err != nil {
			fmt.Fprintf(errOut, "Error: push failed: %v\n", err)
			return ExitError
		}
		...
		if resp.RolledBack {
			fmt.Fprintln(errOut, "ROLLED BACK: One or more rules failed, all changes reverted")
		...
		if len(resp.Results) != len(toPush) {
			fmt.Fprintf(errOut, "Warning: result count (%d) does not match rule count (%d), skipping lockfile update\n",
				...
				if result.Action == "created" || result.Action == "updated" {
					if item.Rule.Rule.ID == "" && result.ID != "" {
						...
						if saveErr := internalyaml.SaveFile(item.Rule.Rule, item.Rule.FilePath); saveErr != nil {
							fmt.Fprintf(errOut, "  Warning: failed to write ID back to %s: %v\n", item.Rule.FilePath, saveErr)
						}
					}
	...
	if wantsTests && len(pushRules) > 0 {
		if !runTests(client, pushRules) {
			testsPassed = false
			if !*forceSync {
				fmt.Fprintln(errOut, "Tests failed. Test definitions were NOT saved to the platform.")
				fmt.Fprintln(errOut, "Fix the tests and re-run, or use -force-sync to save anyway.")
				return ExitError
			}
			fmt.Fprintln(errOut, "Warning: continuing despite test failures (-force-sync)")
		}
	}
	...
	if wantsTests && len(pushedAPIRules) > 0 && (testsPassed || *forceSync) {
		...
		if hasTests {
			...
			if err != nil {
				fmt.Fprintf(errOut, "Warning: failed to save test definitions: %v\n", err)
			}
		}
	}
	...
	if len(toPull) > 0 {
		...
		if err != nil {
			fmt.Fprintf(errOut, "Error: pull failed: %v\n", err)
			return ExitError
		}
	...
	if *deploy {
		...
		if len(deployIDs) > 0 {
			...
			if err != nil {
				fmt.Fprintf(errOut, "Error: deploy failed: %v\n", err)
				if !*forceDeploy {
					return ExitError
				}
				fmt.Fprintln(errOut, "Warning: deploy failed but continuing (-force-deploy)")
			} else {
				...
				if deployResp.Failed > 0 && !*forceDeploy {
					fmt.Fprintf(errOut, "Deploy failed for %d rules\n", deployResp.Failed)
					return ExitError
				}
