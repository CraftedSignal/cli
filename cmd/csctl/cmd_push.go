...
func cmdPush(url, token string, args []string, cfg *config.Config, clientOpts []api.ClientOption, rulePath string) int {
	...
	if url == "" || token == "" {
		fmt.Fprintln(errOut, "Error: URL and token required (set via .csctl.yaml and CSCTL_TOKEN)")
		return ExitError
	}
	...
	if len(loadErrors) > 0 {
		if !*forceSync {
			fmt.Fprintf(errOut, "Error: failed to load rules: %v\n", loadErrors[0])
			return ExitError
		}
	...
	if err != nil {
		fmt.Fprintf(errOut, "Error: import failed: %v\n", err)
		return ExitError
	}
