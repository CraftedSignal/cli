...
func runTests(client *api.Client, rules []internalyaml.LoadedRule) bool {
	...
	if len(startedIDs) == 0 {
		if resp.Errors > 0 {
			_, _ = fmt.Fprintln(errOut, "All test triggers failed")
			return false
		}
				...
				case "failing":
					...
					for _, f := range result.FailedTests {
						if f.Error != "" {
							...
							_, _ = fmt.Fprintf(errOut, "       - %s (%s): expected match, got none\n", f.Name, f.Type)
						} else if f.Type == "negative" && f.Matches > 0 {
							_, _ = fmt.Fprintf(errOut, "       - %s (%s): expected no match, got %d\n", f.Name, f.Type, f.Matches)
						} else {
							_, _ = fmt.Fprintf(errOut, "       - %s (%s): failed\n", f.Name, f.Type)
						}
					}
				case "error":
					allPassed = false
	...
	_, _ = fmt.Fprintln(errOut, "Error: test execution timed out after 5 minutes")
	return false
}
...
func logImportStatus(statusCode int) {
	...
	case http.StatusConflict:
		_, _ = fmt.Fprintln(errOut, "Warning: server reported conflicts (409)")
	case http.StatusUnprocessableEntity:
		_, _ = fmt.Fprintln(errOut, "Warning: server rejected some rules (422)")
	case http.StatusOK, 207:
		// Normal
	default:
		if statusCode != 0 {
			_, _ = fmt.Fprintf(errOut, "Warning: unexpected status %d from server\n", statusCode)
		}
	}
}
