...
func cmdGenerate(url, token string, args []string, cfg *config.Config, clientOpts []api.ClientOption, path string) int {
	...
	if *fromTI == "" {
		_, _ = fmt.Fprintf(errOut, "Error: --from-ti is required\n")
		fs.Usage()
		return ExitError
	}
	...
	if targetPlatform == "" {
		_, _ = fmt.Fprintf(errOut, "Error: --platform is required (or set CSCTL_PLATFORM in config)\n")
		return ExitError
	}
	...
	if isSigmaFile(*fromTI) {
		...
		if err != nil {
			_, _ = fmt.Fprintf(errOut, "Error reading Sigma file: %v\n", err)
			return ExitError
		}
	...
	if err := os.MkdirAll(outputDir, 0755); err != nil {
		_, _ = fmt.Fprintf(errOut, "Error creating directory %s: %v\n", outputDir, err)
		return ExitError
	}
	...
	for _, d := range rules {
		...
		if err := yamlutil.SaveFile(d, filePath); err != nil {
			_, _ = fmt.Fprintf(errOut, "Error saving %s: %v\n", filePath, err)
			continue
		}
		fmt.Printf("  Saved: %s\n", filePath)
	}
