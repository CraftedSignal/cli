...
func cmdInit(url, token string, args []string, clientOpts []api.ClientOption, rulePath string) int {
	...
	if *fromPlatform {
		if url == "" || token == "" {
			_, _ = fmt.Fprintln(errOut, "Error: URL and token required for --from-platform")
			return ExitError
		}
	...
	for _, dir := range dirs {
		if err := os.MkdirAll(dir, 0755); err != nil {
			_, _ = fmt.Fprintf(errOut, "Error: failed to create %s: %v\n", dir, err)
			return ExitError
		}
	...
	if err := internalyaml.SaveFile(example, examplePath); err != nil {
		_, _ = fmt.Fprintf(errOut, "Error: failed to create example: %v\n", err)
		return ExitError
	}
	fmt.Printf("  Created %s\n", examplePath)
...
`
		if err := os.WriteFile(".csctl.yaml", []byte(configContent), 0600); err != nil {
			_, _ = fmt.Fprintf(errOut, "Error: failed to create .csctl.yaml: %v\n", err)
			return ExitError
		}
